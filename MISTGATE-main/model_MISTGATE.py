...
class MISTGATEModel():
    ...
    def __init__(self, hidden_dims1, hidden_dims2, spot_num,  temp=1.0,
                 nonlinear=True, weight_decay=0.0001, dual_graph_weight=0.0,
                 triplet_lambda=0.0, triplet_margin=0.5,
        ...
        self.temp = temp
        self.dual_graph_weight = float(dual_graph_weight)
    ...
    def __call__(self, A, prune_A, GP, X1, X2,
                 triplet_anchors=None, triplet_positives=None, triplet_negatives=None):
        # Encoder
        ...
        self.H_joint = tf.nn.l2_normalize((RNA_e + ATAC_e) / 2.0, dim=1)
        ...
        triplet_loss = self.__triplet_loss(
            self.H_joint, triplet_anchors, triplet_positives, triplet_negatives)
        consistency_loss = self.__similarity_consistency_loss(self.H1, self.H2)

        self.loss = (features_loss1 + features_loss2 + weight_decay_loss + clip_loss +
                     self.triplet_lambda * triplet_loss +
        ...
        return self.loss, self.loss_rna, self.loss_atac, self.weight_decay_loss, self.clip_loss, \
               self.triplet_loss, self.consistency_loss, self.H1, self.H2, \
    ...
    def __encoder(self, A, prune_A, H, W, C, prune_C, v, prune_v, layer, k):
        ...
        spatial_H = tf.sparse_tensor_dense_matmul(C[layer], H)
        if self.dual_graph_weight <= 0:
            return spatial_H

        prune_C[layer] = self.graph_attention_layer(prune_A, H, v[layer], layer, "feature_%s" % k)
        feature_H = tf.sparse_tensor_dense_matmul(prune_C[layer], H)
        return (1.0 - self.dual_graph_weight) * spatial_H + self.dual_graph_weight * feature_H
    ...
    def __decoder(self, H, W, C, prune_C, layer):
        ...
        spatial_H = tf.sparse_tensor_dense_matmul(C[layer - 1], H)
        if self.dual_graph_weight <= 0 or (layer - 1) not in prune_C:
            return spatial_H

        feature_H = tf.sparse_tensor_dense_matmul(prune_C[layer - 1], H)
        return (1.0 - self.dual_graph_weight) * spatial_H + self.dual_graph_weight * feature_H
    ...
    def graph_attention_layer(self, A, M, v, layer, k):
        ...
        with tf.variable_scope("layer_%s" % layer + str(k)):
            ...
            unnormalized_attentions = tf.SparseTensor(indices=logits.indices,
                                                      values=tf.log(tf.maximum(A.values, 1e-8)) * tf.nn.sigmoid(logits.values),
    ...
    def __triplet_loss(self, embedding, anchors, positives, negatives):
        if self.triplet_lambda <= 0 or anchors is None or positives is None or negatives is None:
            return tf.constant(0.0, dtype=tf.float32)

        anchors = tf.cast(anchors, tf.int32)
        positives = tf.cast(positives, tf.int32)
        negatives = tf.cast(negatives, tf.int32)

        def compute_loss():
            anchor_embed = tf.gather(embedding, anchors)
            positive_embed = tf.gather(embedding, positives)
            negative_embed = tf.gather(embedding, negatives)
            ...
            return tf.reduce_mean(tf.maximum(pos_dist - neg_dist + self.triplet_margin, 0.0))

        return tf.cond(tf.greater(tf.size(anchors), 0),
                       compute_loss,
                       lambda: tf.constant(0.0, dtype=tf.float32))

    def __similarity_consistency_loss(self, H1, H2):
        ...
        return tf.reduce_mean(tf.square(sim1 - sim2))


# Backward-compatible class name for old notebooks.
MGATE = MISTGATEModel
