...
def mclust_python(adata, num_cluster, used_obsm='MISTGATE_clip_all',
                  key_added='mclust', random_seed=2020):
    """Python-only Gaussian mixture replacement for scripts that should not call R/mclust."""
    embedding = np.asarray(adata.obsm[used_obsm], dtype=np.float32)
    ...
    adata.obs[key_added] = (labels + 1).astype(int).astype(str)
    adata.obs[key_added] = adata.obs[key_added].astype('category')
    return adata


def wnn_python(adata1, adata2, res=0.5, algo=1, used_obsm='MISTGATE',
               key_added='wnn', n_neighbors=15, random_seed=2020,
               n_clusters=None, spatial_weight=0.0, spatial_key='Spatial_Net'):
    """Python-only WNN-style integration and clustering.

    This keeps the experiment independent from Seurat/rpy2. It builds a
    lightweight WNN graph from the two MIST-GATE embeddings, clusters that
    graph with Scanpy's Python Louvain implementation when available, and
    falls back to Leiden if Louvain is unavailable. It does not use centroid fallback clustering.
    """
    del algo  # kept for API compatibility with wnn_R
    del n_clusters  # kept for API compatibility; graph clustering controls cluster count.
    rna_embedding = normalize(np.asarray(adata1.obsm[used_obsm], dtype=np.float32), norm='l2')
    modality_embedding = normalize(np.asarray(adata2.obsm[used_obsm], dtype=np.float32), norm='l2')

    graph1, dist1 = _knn_affinity_graph_np(rna_embedding, n_neighbors=n_neighbors)
    graph2, dist2 = _knn_affinity_graph_np(modality_embedding, n_neighbors=n_neighbors)

    local1 = np.maximum(np.mean(dist1[:, 1:], axis=1), 1e-8)
    local2 = np.maximum(np.mean(dist2[:, 1:], axis=1), 1e-8)
    score1 = 1.0 / local1
    score2 = 1.0 / local2
    weight1 = score1 / (score1 + score2 + 1e-8)
    weight2 = 1.0 - weight1

    wnn_graph = sp.diags(weight1.astype(np.float32)).dot(graph1) + \
        sp.diags(weight2.astype(np.float32)).dot(graph2)
    wnn_graph = _symmetrize_max_np(wnn_graph).tocsr()
    wnn_graph.eliminate_zeros()
    spatial_weight = float(np.clip(spatial_weight, 0.0, 1.0))
    if spatial_weight > 0:
        spatial_graph = _spatial_affinity_graph_from_uns_np(adata1, spatial_key=spatial_key)
        if spatial_graph is not None and spatial_graph.nnz > 0:
            wnn_graph = (1.0 - spatial_weight) * _scale_graph_unit_max_np(wnn_graph) + \
                spatial_weight * _scale_graph_unit_max_np(spatial_graph)
            wnn_graph = _symmetrize_max_np(wnn_graph).tocsr()
            wnn_graph.eliminate_zeros()

    wnn_embedding = normalize(
        np.concatenate([weight1[:, None] * rna_embedding,
                        weight2[:, None] * modality_embedding], axis=1),
        norm='l2'
    )

    adata1.obsp['wnn_connectivities'] = wnn_graph
    adata2.obsp['wnn_connectivities'] = wnn_graph
    adata1.obsm['X_wnn_python'] = wnn_embedding
    adata2.obsm['X_wnn_python'] = wnn_embedding
    ...
    try:
        ...
        try:
            ...
            raise RuntimeError(
                "Python WNN clustering requires Scanpy Louvain or Leiden graph clustering. "
                "Install `louvain` or `leidenalg`, or call a clustering method explicitly after "
                "`wnn_python` builds `adata.obsp['wnn_connectivities']` and "
                "`adata.obsm['X_wnn_python']`."
            ) from leiden_error

    adata2.obs[key_added] = adata1.obs[key_added].values
    adata2.obs[key_added] = adata2.obs[key_added].astype('category')

    try:
        sc.pp.neighbors(
            adata1,
            use_rep='X_wnn_python',
            n_neighbors=n_neighbors,
            key_added='wnn_python',
            random_state=random_seed,
        )
        sc.tl.umap(adata1, neighbors_key='wnn_python', random_state=random_seed)
        adata2.obsm['X_umap'] = adata1.obsm['X_umap'].copy()
    except Exception:
        adata1.obsm['X_umap'] = _fallback_2d_embedding(wnn_embedding, random_seed=random_seed)
        adata2.obsm['X_umap'] = adata1.obsm['X_umap'].copy()

    return adata1, adata2


def _fallback_2d_embedding(embedding, random_seed=2020):
    if embedding.shape[1] >= 2:
        return PCA(n_components=2, random_state=random_seed).fit_transform(embedding)
    return np.concatenate([embedding, np.zeros((embedding.shape[0], 1), dtype=embedding.dtype)], axis=1)


def _knn_affinity_graph_np(embedding, n_neighbors=15):
    ...
    graph = _symmetrize_max_np(graph)
    graph.eliminate_zeros()
    return graph.tocoo(), distances


def _spatial_affinity_graph_from_uns_np(adata, spatial_key='Spatial_Net'):
    if spatial_key not in adata.uns:
        return None

    spatial_net = adata.uns[spatial_key]
    if not {'Cell1', 'Cell2'}.issubset(spatial_net.columns):
        return None

    obs_to_idx = {cell: idx for idx, cell in enumerate(adata.obs_names)}
    rows = spatial_net['Cell1'].map(obs_to_idx)
    cols = spatial_net['Cell2'].map(obs_to_idx)
    valid = rows.notna() & cols.notna() & (rows != cols)
    if valid.sum() == 0:
        return None

    rows = rows[valid].astype(np.int64).to_numpy()
    cols = cols[valid].astype(np.int64).to_numpy()
    if 'Distance' in spatial_net.columns:
        distances = spatial_net.loc[valid, 'Distance'].astype(np.float32).to_numpy()
        positive = distances[distances > 0]
        sigma = np.median(positive) if positive.size > 0 else 1.0
        values = np.exp(-distances / (sigma + 1e-8)).astype(np.float32)
    else:
        values = np.ones(rows.shape[0], dtype=np.float32)

    graph = sp.coo_matrix((values, (rows, cols)), shape=(adata.n_obs, adata.n_obs))
    graph = _symmetrize_max_np(graph).tocsr()
    graph.eliminate_zeros()
    return graph


def _scale_graph_unit_max_np(graph):
    graph = graph.tocsr().astype(np.float32)
    if graph.nnz > 0:
        max_value = np.max(graph.data)
        if max_value > 0:
            graph.data = graph.data / max_value
    return graph


def _symmetrize_max_np(graph):
    graph = graph.tocsr()
    return graph.maximum(graph.T).tocoo()


def wnn_R(adata1, adata2, res=0.5, algo =  1, used_obsm='MISTGATE'):
    """
    Clustering using the mclust algorithm.
    The parameters are the same as those in the R package mclust.
    """
    ...
    return adata1, adata2
def wnn_protein(adata1, adata2, res=0.5, algo =  1, used_obsm='MISTGATE'):
    """
    Clustering using the mclust algorithm.
    The parameters are the same as those in the R package mclust.
    """
