...
def window_graph(
    ...
    left_df = _as_bed_dataframe(left)
    right_df = _as_bed_dataframe(right)
    if not left_sorted:
        left_df = left_df.sort_values(["chrom", "chromStart", "chromEnd"])
    if not right_sorted:
        right_df = right_df.sort_values(["chrom", "chromStart", "chromEnd"])

    right_by_chrom = {chrom: df for chrom, df in right_df.groupby("chrom", sort=False)}
    for _, left_row in left_df.iterrows():
        right_chr = right_by_chrom.get(left_row["chrom"])
        if right_chr is None or right_chr.empty:
            continue

        candidates = right_chr.loc[
            (right_chr["chromStart"] <= left_row["chromEnd"] + window_size) &
            (right_chr["chromEnd"] >= left_row["chromStart"] - window_size)
        ]
        if candidates.empty:
            continue

        left_interval = _row_to_interval(left_row)
        for _, right_row in candidates.iterrows():
            right_interval = _row_to_interval(right_row)
            d = interval_dist(left_interval, right_interval)
            if -window_size <= d <= window_size:
                graph.add_edge(left_interval.name, right_interval.name,
                               **attr_fn(left_interval, right_interval, d))

    return graph


def _as_bed_dataframe(data: Union[Bed, str]) -> pd.DataFrame:
    if isinstance(data, Bed):
        return pd.DataFrame(data, copy=True)
    return pd.DataFrame(Bed.read_bed(data), copy=True)


def _row_to_interval(row: pd.Series) -> Interval:
    return Interval(
        str(row["chrom"]),
        int(row["chromStart"]),
        int(row["chromEnd"]),
